...
export class WalletController {
  ...
  /**
   * Add manual credits (admin function)
   * POST /wallet/add-credits
   */
  addCredits = async (req: Request, res: Response): Promise<void> => {
    logger.request('POST', '/wallet/add-credits', req.body);
    ...
    try {
      ...
      // For demo purposes, allow any role to add credits
      // In production, this would be admin-only
      const { freelancer_id, amount, description } = req.body;
      
      if (!freelancer_id || !amount || !description) {
        const errorResponse = { error: 'freelancer_id, amount, and description are required' };
        res.status(400).json(errorResponse);
        return;
      }
