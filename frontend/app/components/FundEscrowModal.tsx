...
import { useCreateStripeCheckoutSessionMutation } from '../store/api/paymentApi'
...
export default function FundEscrowModal({ project, onClose, onSuccess }: FundEscrowModalProps) {
  ...
  const handlePay = async () => {
    setError('')
    try {
      const result = await createCheckoutSession({ projectId: project.project_id }).unwrap()
      // Redirect to Stripe hosted checkout
      window.location.href = result.url
    } catch (err: any) {
      setError(err?.data?.error || 'Failed to start payment. Please try again.')
    }
  }
          ...
          <div className="p-6 space-y-5">
            <div className="bg-gray-50 rounded-lg p-4">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="font-bold text-[#AD7D56] text-base">{displayAmount}</span>
              </div>
            </div>
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 text-sm text-blue-800">
              ...
              <p>
                Funds are locked in escrow after payment. They are automatically released to the
                freelancer only when milestones pass AI quality checks.
              </p>
            </div>
            ...
            <div className="flex gap-3 pt-1">
              ...
              <button
                ...
                className="flex-1 px-4 py-2 bg-[#AD7D56] text-white rounded-lg hover:bg-[#8B6344] transition-colors disabled:opacity-50 font-medium flex items-center justify-center gap-2"
              >
                {isLoading ? (
                  <>
